...
namespace PetriEngine
{
    ...
    uint32_t PetriNet::outArc(uint32_t transition, uint32_t place) const
    {
        ...
        for (; imin < imax; ++imin)
        {
            if (_invariants[imin].place == place)
                return _invariants[imin].tokens;
        }
        return 0;
    }
    ...
    void PetriNet::sort()
    {
        for (size_t i = 0; i < _ntransitions; ++i)
        {
            ...
            std::sort(&_invariants[t.inputs], &_invariants[t.outputs], [](const auto &a, const auto &b)
                      { return a.place < b.place; });
            TransPtr &t2 = _transitions[i + 1];
            std::sort(&_invariants[t.outputs], &_invariants[t2.inputs], [](const auto &a, const auto &b)
                      { return a.place < b.place; });
        }
    }
