...
namespace PetriEngine::PQL
{
    ...
    void ConstraintVisitor::_commutative_cons(const CommutativeExpr *element, int _constant, const std::function<void(Member &a, Member b)> &op)
    {
        ...
        {
            if (first)
                res = memberForPlace(i.first, _context);
            else
                op(res, memberForPlace(i.first, _context));
            first = false;
        }
    ...
    {
        _commutative_cons(element, 0, [](auto &a, auto b)
                          { a += b; });
    }
    ...
    {
        _commutative_cons(element, 1, [](auto &a, auto b)
                          { a *= b; });
    }
    ...
    void Simplifier::_accept(const UnfoldedUpperBoundsCondition *element)
    {
        ...
        {
            next.clear();
            RETURN(Retval(std::make_shared<UnfoldedUpperBoundsCondition>(next, 0, bounds[nplaces].first + element->getOffset())))
        }
    ...
    void Simplifier::_accept(const ControlCondition *condition)
    {
        ...
        if (_return_value.formula->isTriviallyTrue() || _return_value.formula->isTriviallyFalse())
        {
            ...
            RETURN(Retval(is_true ? Retval(BooleanCondition::TRUE_CONSTANT) : Retval(BooleanCondition::FALSE_CONSTANT)))
        }
        else
        {
            RETURN(Retval(std::make_shared<ControlCondition>(_context.negated() ? std::make_shared<NotCondition>(_return_value.formula) : _return_value.formula)))
        }
    }
    ...
    void Simplifier::_accept(const EUCondition *condition)
    {
        ...
        if (r2.formula->isTriviallyTrue() || !r2.neglps->satisfiable(_context))
        {
            ...
            RETURN(neg ? Retval(BooleanCondition::FALSE_CONSTANT) : Retval(BooleanCondition::TRUE_CONSTANT))
        }
        else if (r2.formula->isTriviallyFalse() || !r2.lps->satisfiable(_context))
        {
            _context.setNegate(neg);
            RETURN(neg ? Retval(BooleanCondition::TRUE_CONSTANT) : Retval(BooleanCondition::FALSE_CONSTANT))
        }
